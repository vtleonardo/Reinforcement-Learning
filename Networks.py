...
def DQN_regularized(state_input_shape, actions_num, name, normalize=True):
    l=0.01
    K_reg = l2(l)
    drop_rate = 0.25
...
def DQN(state_input_shape, actions_num, name, normalize = True):
    input = Input(state_input_shape, name=name)
    if normalize:
        lamb = Lambda(lambda x: (2 * x - 255) / 255.0, )(input)
...
def DRQN(state_input_shape, actions_num, name, normalize=True):
