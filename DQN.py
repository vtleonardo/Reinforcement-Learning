...
class AgentDQN:
    ...
    def __init__(self,
        ...
        :param:
            ...
            load : bool (Default: False)
                Variable that controls if it's to load the weights from a external .h5 file generated by another
                simualtion.
            step_save_weights : int (Default: 50 000)
                Desired number of frames to save the weights. It should be multiple of the target_update
            ...
            multi_gpu : bool (Default : False)
                If false only the first gpu will be used if any.
        """
        ...
        self.steps_save_weights = step_save_weights
        ...
        printd("==================================================================================================")
        printd("INITIALIZING THE DQN ALGORITHM WITH THE FOLLOW SETTINGS:")
        printd("    Environment: {}".format(self.env.getName()))
        ...
        printd("    Approximate number of states from random plays before training: {} state(s)".format(self.num_random_play))
        ...
        if load:
            printd("Load the weights is set to True!")
            printd("The weights {} will be loaded from the {} folder ".format(weights_load_name,path_save_weights))
        printd("    The weights will be saved each: {} frame(s)".format(self.steps_save_weights))
        printd("    Multi gpu mode : {}".format(multi_gpu))
        printd("==================================================================================================")
    ...
    def initalize_network(self, name,load=False,weights_load_name=""):
        ...
        if(load):
            if weights_load_name != "":
                ...
                model.load_weights(os.path.join(self.path_save_weights,weights_load_name))
            else:
    ...
    def save_agent(self):
        """
        Function that saves the variables to plot in a .csv file and the weights in .h5 file. The weights are saved
        each N steps (defined by steps_save_weights on initialization).
        :param
            nothing
        :return
            nothing
        ...
        while self.steps_cont < self.num_simul_frames:
            ...
            printd("Episode {:d}, Total Frames:{:d}/{:d}, Frames in this episode:{:d} , Total reward :{:.2f}, "
                   ...
                   "Time of this episode:{:.3f}(s)".format(self.i_episode,
                                        ...
                                        self.reward_100,self.epsilon,self.replay_memory.size(),
                                                avg_loss,avg_q_rate,fps,time.time()-time_it),lvl=1)
            time_it = time.time()
